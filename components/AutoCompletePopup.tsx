...
const AutoCompletePopup: React.FC<AutoCompletePopupProps> = ({ commands, onSelect, selectedIndex }) => {
  return (
    <div className="absolute bottom-full mb-2 w-full bg-surface border border-secondary/50 rounded-lg shadow-2xl z-10">
      <div className="p-2 text-xs text-secondary font-semibold">COMMANDS</div>
      <ul className="max-h-60 overflow-y-auto custom-scrollbar">
        {commands.map((cmd, index) => (
          <li
            key={cmd.name}
            className={`px-3 py-2 cursor-pointer flex items-center gap-3 ${index === selectedIndex ? 'bg-accent/20' : 'hover:bg-background/50'}`}
